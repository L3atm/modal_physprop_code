...
!BSINGH - Endrun is now part of the optics_support file
!use abortutils, only: endrun
use module_optics_support, only: endrun
...
real(r8), parameter, public :: wavenumber1_longwave(nlwbands) = &! Longwave spectral band limits (cm-1) !BSINGH - Made public
...
subroutine get_lw_spectral_boundaries(low_boundaries, high_boundaries, units)
   ...
   case default
      call endrun('get_lw_spectral_boundaries: spectral units not acceptable')!
   end select
...
subroutine get_sw_spectral_boundaries(low_boundaries, high_boundaries, units)
   ...
   case default
      call endrun('rad_constants.F90: spectral units not acceptable')!
   end select
...
integer function rad_gas_index(gasname)
   ...
   enddo
   call endrun ("rad_gas_index: can not find gas with name ")!
